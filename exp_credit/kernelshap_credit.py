...
if __name__ == "__main__":
  ...
  if args.softmax:
    ...
    save_checkpoint_name = "../credit-dataset/softmax/kernelshap_credit_m_1_s_" + str(args.sample_num) + "_r_" + str(checkpoint["round_index"]) + "_c_" + str(args.circ_num) + ".pth.tar"

  else:
    imputer = removal.MarginalExtension(reference, model_for_shap.forward_1_np)
    save_checkpoint_name = "../credit-dataset/wo_softmax/kernelshap_credit_m_1_s_" + str(args.sample_num) + "_r_" + str(checkpoint["round_index"]) + "_c_" + str(args.circ_num) + ".pth.tar"
