  ...
  for index, (x, y, z, sh_gt, rk_gt) in enumerate(data_loader):
    ...
    for idx in range(args.circ_num * feat_num):
      ...
      attr = shapley.ShapleyRegression(
          ...
          n_samples=args.sample_num // 2, batch_size=args.sample_num // 2,
          bar=False)
    ...
    # attr = eff_shap(model.forward, z, reference, error_matrix, topK)
...
if __name__ == "__main__":
  ...
  if args.softmax:
    ...
    save_checkpoint_name = "../credit-dataset/softmax/kernelshap_pair_credit_m_1_s_" + str(args.sample_num) + "_r_" + str(checkpoint["round_index"]) + "_c_" + str(args.circ_num) + ".pth.tar"

  else:
    imputer = removal.MarginalExtension(reference, model_for_shap.forward_1_np)
    save_checkpoint_name = "../credit-dataset/wo_softmax/kernelshap_pair_credit_m_1_s_" + str(args.sample_num) + "_r_" + str(checkpoint["round_index"]) + "_c_" + str(args.circ_num) + ".pth.tar"

  shapley_value, shapley_rank, total_time = shapreg_shapley(imputer, data_loader)  # , shapley_value_gt, shapley_ranking_gt)
