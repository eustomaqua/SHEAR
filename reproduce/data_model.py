...
class mlp(nn.Module):
  def __init__(self, input_dim, output_dim,
               layer_num=1, hidden_dim=64, activation=None):
  ...
  def forward_wo_sigmoid(self, x):
    # only in `adult_model.py`
    return self.forward_softmax(x)[:, 1].unsqueeze(
        dim=1) - self.forward_softmax(x)[:, 0].unsqueeze(dim=1)
...
class Model_for_shap(nn.Module):
  ...
  def forward(self, x):
    ...
    for sf_index in range(x_sparse_feature.shape[1]):
      ...
      feature_i[feature_i == -1] = sparse_codebookping.shape[0] - 1
      # -1 for background noise
      index = feature_i.unsqueeze(
          dim=1).repeat((1, sparse_codebookping.shape[-1]))
